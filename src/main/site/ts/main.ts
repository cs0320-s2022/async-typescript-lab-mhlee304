...
function postAndUpdate(): void {
  ...
  const postParameters: MatchesRequestData = {sun: sun.value, moon: moon.value, rising: rising.value
...
function updateSuggestions(matches: string[]): void {
  ...
  while(i < matches.length) {
    let curr = matches[i]
    suggestions.innerHTML += `<li tabindex="0">${curr}</li>`
    i++
    console.log(i)
  }
...
document.addEventListener('keyup', async(key) =>{
  ...
  await updateValues(sun.value, moon.value, rising.value);
  postAndUpdate();
})
